...
- 按主机保存 Pi 快捷任务模板，通过 `{{input}}` 填入少量参数后一键创建并进入会话；每个任务可独立配置模型（支持 `provider/model`）和 Thinking effort，留空/默认时沿用 Pi 配置，档位支持取决于模型及远程 Pi 版本
- 模型选择器在当前 SSH 主机的任务目录执行 `pi --list-models`，按厂商分组、支持搜索，显示上下文/输出长度及 thinking/图片能力；按连接和目录缓存，可手动刷新，也可手动输入模型。列表反映远程 Pi 已配置凭据的可用模型，不保证凭据有效或账号调用权限
